...
```{r}
mean_iq <- round(mean(our_data$IQ), 2)
mean_eq <- round(mean(our_data$EQ), 2)
...
```{r echo=F}
ss_iq <- round(sum((our_data$IQ-mean(our_data$IQ))^2), 2)
ss_eq <- round(sum((our_data$EQ-mean(our_data$EQ))^2), 2)


sp_iqeq <- round(sum((our_data$IQ-mean(our_data$IQ))*(our_data$EQ-mean(our_data$EQ))), 2)
...
```{r eval=cor_ans=="Yes"}
cor.test(our_data$IQ, our_data$EQ)
```
...
```{r}
p_value <- ifelse(cor.test(our_data$IQ, our_data$EQ)$p.value<.001, .001, round(cor.test(our_data$IQ, our_data$EQ)$p.value, 3))
r_value <- round(cor.test(our_data$IQ, our_data$EQ)$estimate, 2)
r_square <- round((cor.test(our_data$IQ, our_data$EQ)$estimate)*(cor.test(our_data$IQ, our_data$EQ)$estimate), 2)
